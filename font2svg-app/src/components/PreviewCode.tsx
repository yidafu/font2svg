...
function CopyIcon(props: { className?: string, text: string }) {
  ...
  return <svg
    onClick={() => {
      onCopy()
      toast({ status: 'success', description: '复制成功', position: 'top', variant: 'subtle' })
    }}
...
export function PreviewCode(props: IPreviewCodeProps) {
  ...
  return <div className='relative'>
    ...
    <SyntaxHighlighter language={props.lang ?? 'jsx'} style={atomDark} showLineNumbers wrapLines wrapLongLines>
      {props.code}
    </SyntaxHighlighter>
    {props.copyable ? <CopyIcon text={props.code} className='absolute top-0 right-0 p-2' /> : null}
  </div>
}