...
export function FontDetail(props: { onBack(): void; fontFaceId: number }) {
  ...
  return (
    <div>
      <Button leftIcon={<BackIcon />} variant="outline" onClick={props.onBack}>
        返回
      </Button>
      <Card>
        <CardHeader>
          ...
          <Link href={fontFace.downloadUrl} download>
            下载字体
          </Link>
        </CardHeader>
        <CardBody>
          <StatGroup>
            ...
            <Stat>
              ...
              <StatNumber>
                {fontFace?.tasks?.[0]?.generateCount ?? 0}个
              </StatNumber>
            </Stat>
          </StatGroup>
        </CardBody>
